...
class UpdateSoldElonJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        if (!$elon || !$bot) {
            Log::error("UpdateSoldElonJob: Elon or Bot not found", [
                'elon_id' => $this->elonId,
                'bot_id' => $this->botId,
            ]);
            return;
        ...
        if (!$elon->elon_message_id) {
            Log::warning("UpdateSoldElonJob: Elon message ID not found", [
                'elon_id' => $this->elonId,
            ]);
            return;
        }
        ...
        if ($elon->cancelled_from_admin || $elon->cancelled_from_user || $elon->status !== Elon::STATUS_COMPLATED) {
            Log::info("UpdateSoldElonJob: Elon cancelled or not completed", [
                'elon_id' => $this->elonId,
                'status' => $elon->status,
            ]);
            return;
        ...
        if (!$bot->channel_id) {
            Log::error("UpdateSoldElonJob: Channel ID not found", [
                'bot_id' => $this->botId,
            ]);
            return;
        }
        ...
        try {
            ...
            $caption = $this->formatElonForChannelSold($elon, $bot);
            ...
            try {
                ...
                if (str_contains($e->getMessage(), 'message is not modified')) {
                    Log::info("UpdateSoldElonJob: Message not modified (content is the same)", [
                        'elon_id' => $this->elonId,
                        'message_id' => $elon->elon_message_id,
                    ]);
                    // Xatolikni ignore qilish - bu normal holat
                    ...
                    Log::warning("UpdateSoldElonJob: Message not found (may be deleted or message_id changed)", [
                        'elon_id' => $this->elonId,
                        'message_id' => $elon->elon_message_id,
                        'error' => $e->getMessage(),
                    ]);
            ...
            // Sticker yuborish (elon xabari yoniga)
            try {
                // "Sotildi" sticker file_id (config'dan olish yoki hardcode)
                $soldStickerId = config('elon.sold_sticker_id', null);
                
                if ($soldStickerId) {
                    $telegram->sendSticker([
                        'chat_id' => $bot->channel_id,
                        'sticker' => $soldStickerId,
                        'reply_to_message_id' => (int) $elon->elon_message_id,
                    ]);
                    
                    Log::info("UpdateSoldElonJob: Sticker sent", [
                        'elon_id' => $this->elonId,
                        'sticker_id' => $soldStickerId,
                    ]);
                } else {
                    Log::info("UpdateSoldElonJob: Sticker ID not configured", [
                        'elon_id' => $this->elonId,
                    ]);
                }
            } catch (\Exception $e) {
                Log::warning("UpdateSoldElonJob: Could not send sticker", [
                    'elon_id' => $this->elonId,
                    'error' => $e->getMessage(),
                ]);
            }

            Log::info("UpdateSoldElonJob: Elon updated successfully", [
                'elon_id' => $this->elonId,
                'channel_id' => $bot->channel_id,
                'message_id' => $elon->elon_message_id,
            ]);
        } catch (\Exception $e) {
            Log::error("UpdateSoldElonJob: Error updating elon", [
                'elon_id' => $this->elonId,
                'error' => $e->getMessage(),
    ...
    /**
     * Kanal uchun elon formatlash (HTML format) - Sotilgan holat
     */
    private function formatElonForChannelSold(Elon $elon, Bot $bot): string
    {
        ...
        try {
            ...
            Log::warning("UpdateSoldElonJob: Failed to get bot username", [
                'bot_id' => $bot->id,
                'error' => $e->getMessage(),
            ]);
        }
